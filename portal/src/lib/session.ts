// Signed session cookie. A client session holds only that client's ID; an admin
// session holds `admin: true` plus the client currently being viewed.
...
import { getClient, getClientForAdmin, listClientsForAdmin } from "@/lib/data";
...
type SessionPayload = { clientId?: string; admin?: true; exp: number };
...
function decode(token: string | undefined): SessionPayload | null {
  ...
  try {
    ...
    if (!payload.admin && typeof payload.clientId !== "string") return null;
    return payload;
  } catch {
    return null;
  }
...
export async function createSession(clientId: string) {
  await write({ clientId }, CLIENT_MAX_AGE);
}
...
type Session = { client: Client | null; isAdmin: boolean };
...
export const getSession = cache(async (): Promise<Session | null> => {
  ...
  // Clients re-check they still have portal access on every request.
